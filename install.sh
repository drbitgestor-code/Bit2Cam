...
# Copiar config-api.py do diretório local (preferido) ou usar cópia embutida
if [[ -f "$SCRIPT_DIR/config-api.py" ]]; then
  cp "$SCRIPT_DIR/config-api.py" "$INSTALL_DIR/config-api.py"
  success "config-api.py copiado da pasta local"
else
...
if __name__ == '__main__':
    ...
    try:
        ...
        sys.exit(0)
PYEOF
  success "config-api.py criado (embutido)"
fi
...
echo -n "$SETUP_HASH" > "$INSTALL_DIR/auth.hash"
success "Hash de autenticação gravado em auth.hash"
